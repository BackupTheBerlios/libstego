...
int test_embed(palette_data_t* palette_data, 
    ...
    printf ("Embedding!\n");
    if (gifshuffle_embed(palette_data, stego_data, message, msg_length, para) != 0) {
    ...
    if (ret = io_gif_cleanup_data(stego_data) != 0) {
        printf("  <-- Cleaning up gif type's stego data failed. Error code: %d\n", ret);
        return 1;
    }
    else {
        printf("  --> Cleaning up gif type's stego data worked.\n");
    }
    return 0;
}
